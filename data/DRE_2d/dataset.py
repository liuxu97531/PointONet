...
def generate_one_training_data(P, m):
    ...
    xx = np.random.rand(P, m)
    u = f_fn(xx)
    u = np.stack([xx, u], axis=1)

    xx1 = np.linspace(xmin, xmax, m)
    u1 = f_fn(xx1)
...
def generate_training_data(N, P, m):
    ...
    train_data_D = []
    for _ in pbar:
        data_out = generate_one_training_data(P, m)
        train_data_P.append(data_out[:3])
        train_data_D.append(data_out[3:])
    u_train = np.array([data[0] for data in train_data_P]).reshape(N * P, 2, -1)
    y_train = np.array([data[1] for data in train_data_P]).reshape(N * P, -1)
    s_train = np.array([data[2] for data in train_data_P]).reshape(N * P, -1)

    u1_train = np.array([data[0] for data in train_data_D]).reshape(N * P, -1)

    return u_train, u1_train, y_train, s_train
...
def generate_test_data(N, P, m):
    ...
    u_test = np.array([data[0] for data in test_data_P]).reshape(N * P ** 2, 2, -1)
    y_test = np.array([data[1] for data in test_data_P]).reshape(N * P ** 2, -1)
    s_test = np.array([data[2] for data in test_data_P]).reshape(N * P ** 2, -1)
    u1_test = np.array([data[0] for data in test_data_D]).reshape(N * P ** 2, -1)
    return u_test, y_test, s_test, u1_test


def generate_one_test_data_uniform(P, m):
    k = lambda x: 0.01 * np.ones_like(x)
    v = lambda x: np.zeros_like(x)
    g = lambda u: 0.01 * u ** 2
    dg = lambda u: 0.02 * u
    ...
    xx = np.linspace(xmin, xmax, m)
    # xx = np.random.rand(1, m)*(xmax-xmin) + xmin
    u = f_fn(xx)
    u_test = np.tile(u, (P ** 2, 1))

    u = np.stack([xx, u], axis=1).T
    u_p_test = np.tile(u, (P ** 2, 1))
...
def generate_test_data_uniform(N, P, m):
    pbar = trange(N, disable=True)
    test_data_P = []
    test_data_D = []
    for _ in pbar:
        data_out = generate_one_test_data_uniform(P, m)
        test_data_P.append(data_out[:3])
        test_data_D.append(data_out[3:])
    u_test = np.array([data[0] for data in test_data_P]).reshape(N * P ** 2, 2, -1)
    y_test = np.array([data[1] for data in test_data_P]).reshape(N * P ** 2, -1)
